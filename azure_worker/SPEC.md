...
| `negative_prompt` | string | no | Max 4000 chars. **Honored on `chroma1` and `qwen-image-2512`** (both support real CFG with a real negative branch). **Ignored** on `flux1-dev` and `fluxed-up` (both use `ConditioningZeroOut`) and `flux2-klein` (uses `BasicGuider` — no negative path). |
...
| `cfg` | number | no | 0.0 ≤ cfg ≤ 30.0. Default 7.0. **Honored on `chroma1`** (recommend 3.5; workable 3.5–7) and **`qwen-image-2512`** (recommend 4.0). **Ignored** on `flux1-dev`, `flux2-klein`, and `fluxed-up` (all guidance-distilled). |
